...
declare const Tesseract: unknown;
...
const IMMUTABLE_IGNORE_LIST = [
    ...
    'VOLUME', 'PESO', 'KG', 'GRAMAS', 'LITROS', // Metadados Fiscais
...
export class ExclusiveScannerService {
  ...
  public async runOCR(image: Blob | string): Promise<string> {
    ...
    try {
        ...
        try {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const workerCDN = await (Tesseract as any).createWorker('por', 1, {
                errorHandler: () => {}
            });
  ...
  public filterBlacklist(text: string): string {
    return text.split('\n')
      .filter(line => !IMMUTABLE_IGNORE_LIST.some(badTerm => line.toUpperCase().includes(badTerm)))
      .join('\n');
  }

  public assistWithRegex(text: string): { rastreio?: string; bloco?: string; apto?: string } {
    const hints: { rastreio?: string; bloco?: string; apto?: string } = {};
    
    const rastreioMatch = text.match(/[A-Z]{2}\d{9}[A-Z]{2}/);
    if (rastreioMatch) hints.rastreio = rastreioMatch[0];
    
    const blocoMatch = text.match(/BLOCO\s*([A-Z0-9]+)/i);
    if (blocoMatch) hints.bloco = blocoMatch[1];
    
    const aptoMatch = text.match(/APTO\s*([A-Z0-9]+)/i);
    if (aptoMatch) hints.apto = aptoMatch[1];
